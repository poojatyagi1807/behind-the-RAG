...
""", unsafe_allow_html=True)
    ...
    rows_data = [
        (
            ...
            "We have a system prompt that says 'answer only from provided context.' But we never user-tested it: do non-technical PMs find the responses clear? Does the grounding instruction make answers sound robotic or overly hedged? We shipped without testing the output quality.",
        ...
        (
            ...
            "We allocate 6,000 tokens for context (of an 8,192 limit). We never measured: what's the average context size for our chunks? Are we ever actually hitting the limit? No telemetry means we don't know if silent truncation is happening in longer sessions.",
        ...
        (
            ...
            "Our system prompt tells the model to stay on RAG topics. But we never red-teamed it — can a user jailbreak it into answering unrelated questions? We shipped guardrails without testing them. A PM should run 10 adversarial prompts before launch.",
        ...
        (
            ...
            "We don't carry conversation history — each query is independent. This is a known limitation we never surfaced to the user. A PM should decide explicitly: is stateless Q&A acceptable for a learning experience, or does forgetting previous questions hurt the educational flow?",
        ...
        (
            ...
            "The builder wrote the system prompt and it's hardcoded in the generation step. No review, no version history, no documented rationale for the instructions. A PM should own this as a product artifact — versioned, reviewed, and updated like any other product copy.",
