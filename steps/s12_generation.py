...
""")
    ...
    rows_data = [
        (
            ...
            "We never defined an output format. The LLM sometimes responds in bullet points, sometimes in paragraphs. For a learning app, consistency matters — users learning a concept expect the same structural format each time. A PM should define: explain in 3 parts — what it is, why it matters, one example.",
        ...
        (
            ...
            "We default to Gemini 2.5 Flash (free). The choice was driven by cost, not by which model best serves non-technical PM learners. A PM should run: which model produces the clearest, most jargon-free explanations for this audience? We never compared models on our query set.",
        ...
        (
            ...
            "We use the model's default temperature. For an educational app where factual accuracy matters more than creativity, a lower temperature (0.2–0.3) is more appropriate. We never set this explicitly — the model occasionally paraphrases retrieved content in ways that introduce inaccuracy.",
        ...
        (
            ...
            "Our system prompt says 'answer only from provided context.' But we never measured how often the model violates it. We have a grounding checker in Step 13 — but we never used those scores to iterate on the system prompt. A PM should tune until grounding consistently exceeds 0.85.",
        ...
        (
            ...
            "We generate the complete response before showing it. No streaming. For a learning app where some answers are long, users wait with no feedback. A PM should have tested: does streaming increase engagement, or does seeing a partial answer confuse learners who are new to AI?",
        ...
        (
            ...
            "No fallback when generation fails. If the model hits a content filter or API timeout, the user sees a generic error. A PM should define: what does the user see when generation fails? 'Try rephrasing your question' is better than a blank screen.",
