...
""")
    ...
    rows_data = [
        (
            ...
            "We never defined an output format. The LLM alternates between bullet points and paragraphs depending on the query — learners get inconsistent structure with no predictable pattern to build on.",
        ...
        (
            ...
            "We defaulted to Gemini 2.5 Flash because it's free. The model choice was a cost decision — we never compared which model produces the clearest, most jargon-free explanations for a non-technical PM audience.",
        ...
        (
            ...
            "We use the model's default temperature. For an educational app, the default is too high — the model occasionally paraphrases retrieved content in ways that shift the meaning, introducing inaccuracy.",
        ...
        (
            ...
            "Our system prompt says 'answer only from provided context' but we never measured compliance. We have a grounding checker in Step 13 — but we never fed those scores back to improve the system prompt.",
        ...
        (
            ...
            "We generate the full response before displaying it. For longer answers, the user waits with no feedback — we never tested whether streaming would improve engagement for a learning context.",
        ...
        (
            ...
            "We have no fallback when generation fails. If the model hits a content filter or the API times out, the user sees a generic error with no guidance on what to do next.",
