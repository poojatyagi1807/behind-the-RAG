...
def render():
    ...
    rows_data = [
        (
            ...
            "We default to Top 10 from search, then let the user pick K via a slider. But we never validated what K produces the best answers for our query types. A PM should run: take 20 questions, measure faithfulness at K=3, K=5, K=10, set the default based on measured quality.",
        ...
        (
            ...
            "No minimum similarity threshold. A query completely outside our KB still returns chunks — just with low scores. We show a warning at score < 0.25 but never validated that 0.25 is the right threshold. A PM should have calibrated this against human judgement.",
        ...
        (
            ...
            "We built hybrid search (dense + BM25 via RRF) from the start. Good call. But we never measured the improvement over dense-only on our specific KB. A PM should require a before/after measurement before shipping hybrid as the default.",
        ...
        (
            ...
            "We always return something — even low-confidence results. We show a warning banner but still pass chunks to the LLM. A PM should have defined: at what score threshold do we withhold results entirely and show 'I don't know about that topic'?",
        ...
        (
            ...
            "HNSW ef_construction=200, M=16 — default settings. We never tuned for our KB size (312 chunks) or latency target. For 312 chunks, even brute-force would be fast. We over-engineered the index for our current scale without measuring whether HNSW gives any benefit.",
