...
SAME MODEL EVALUATING ITSELF: {gen_model == judge_model}"""
    ...
    rows_data = [
        (
            ...
            "We run all four RAGAS metrics on every evaluation. For a learning app, faithfulness matters most — are we teaching correct RAG concepts? We never prioritised metrics by what matters for our use case. A PM should define: faithfulness is our primary metric; the others are secondary.",
        ...
        (
            ...
            "We show RAGAS scores but never defined what score triggers a change. A PM should set: if faithfulness drops below 0.80 on any standard query, review the system prompt and retrieved chunks before the next user session. No threshold means scores are just numbers with no action attached.",
        ...
        (
            ...
            "We have no golden dataset. We evaluate on whatever query the user happens to type. A PM should have defined a 20-question golden dataset — questions representative of what a non-technical PM would ask — and run RAGAS against those after every KB update.",
        ...
        (
            ...
            "RAGAS scores are shown in the UI but explained only technically. A PM should translate each score into plain English for any stakeholder sharing: 'Faithfulness 0.91 means 91% of what the AI said came directly from our 5 source documents — not invented.'",
        ...
        (
            ...
            "We run RAGAS on demand only — when the user clicks Evaluate. No offline evaluation runs before a KB update. A PM should set up: run the 20-question golden dataset after every document change, before the app is shared with new users.",
        ...
        (
            ...
            "We have no process to connect RAGAS scores to roadmap decisions. If context precision drops, who decides what to fix and when? A PM should define: each RAGAS metric maps to a specific pipeline owner, and a score drop below threshold creates a roadmap item within 48 hours.",
