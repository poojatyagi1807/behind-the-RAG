...
SAME MODEL EVALUATING ITSELF: {gen_model == judge_model}"""
    ...
    rows_data = [
        (
            ...
            "We run all four RAGAS metrics without prioritising them. For a learning app, faithfulness matters most — but we present all metrics equally with no signal that some matter more than others for our use case.",
        ...
        (
            ...
            "We display RAGAS scores with no defined action thresholds. A faithfulness score of 0.70 appears next to one of 0.95 with no indication that 0.70 should trigger a review — the numbers inform nothing.",
        ...
        (
            ...
            "We have no golden dataset. Evaluation runs on whatever query the user happens to type — so our quality measurement is entirely dependent on what an individual user decides to ask that session.",
        ...
        (
            ...
            "We display RAGAS scores with technical labels only. 'Faithfulness: 0.91' appears with no plain-English translation — a non-technical PM sharing results with their team can't explain what the number means.",
        ...
        (
            ...
            "We run RAGAS on demand only. There's no evaluation before a KB update, no scheduled check, no baseline to compare against — quality measurement is reactive, triggered by users, not by us.",
        ...
        (
            ...
            "RAGAS scores are shown in the UI with no process for acting on them. If context precision drops, there's no owner, no threshold, and no roadmap path — the data goes nowhere.",
