...
def render():
    ...
    rows_data = [
        (
            ...
            "We chose all-MiniLM-L6-v2 because it runs locally with no API key — a deployment constraint, not a quality decision. A PM should have evaluated: does this model score well on educational Q&A content? We never benchmarked it against our actual queries.",
        ...
        (
            ...
            "English only — never documented. If this app gets used by non-English speaking PMs (likely, given the global PM audience), retrieval quality on non-English queries is unknown. A PM should make this an explicit documented constraint with a plan to address it.",
        ...
        (
            ...
            "No re-embedding plan. If we add new documents, we need to re-embed everything. We never defined what triggers a re-embed or how long it takes. On Streamlit Cloud, re-embedding 300+ chunks on first load already causes user-visible delays.",
        ...
        (
            ...
            "We picked the free local model. The cost tradeoff was never evaluated. A PM should have estimated: what would OpenAI embeddings cost for this KB at 1,000 sessions/month? Would the quality difference justify it? We skipped that comparison entirely.",
        ...
        (
            ...
            "We're locked to all-MiniLM-L6-v2 via fastembed. If we want to upgrade to a better model, we need to re-embed the entire KB and rebuild the HNSW index. No migration plan exists. A PM should have documented this as a known technical debt item from day one.",
