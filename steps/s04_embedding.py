...
def render():
    ...
    rows_data = [
        (
            ...
            "We chose all-MiniLM-L6-v2 because it runs locally with no API key — a deployment convenience, not a quality decision. We never tested whether it retrieves well on educational Q&A content for a non-technical PM audience.",
        ...
        (
            ...
            "We built English-only with no documentation of that decision. If a non-English-speaking PM uses this app, retrieval quality on their queries is unknown — and there's no error message telling them why answers are poor.",
        ...
        (
            ...
            "We have no re-embedding plan. Adding new documents means re-embedding everything, and we never defined what triggers it or how long it takes — on Streamlit Cloud it already causes a visible delay on every first load.",
        ...
        (
            ...
            "We picked the free local model without evaluating the cost-quality tradeoff. We never estimated what OpenAI embeddings would cost for this KB or whether the quality difference would be worth it.",
        ...
        (
            ...
            "We're locked to all-MiniLM-L6-v2 via fastembed with no migration plan. Upgrading to a better model means re-embedding the entire KB and rebuilding the HNSW index — a known constraint we never documented.",
