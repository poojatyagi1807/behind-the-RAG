...
""", unsafe_allow_html=True)
    ...
    rows_data = [
        (
            ...
            "We use all-MiniLM-L6-v2 for both query and document embedding via the same helper — a code convention, not a version-locked config. If fastembed is upgraded and the model changes silently, retrieval breaks with no alert.",
        ...
        (
            ...
            "We set no latency budget for query embedding. all-MiniLM-L6-v2 takes ~200ms on first call (model load) — we never measured this, set a target, or tracked whether it was acceptable to users.",
        ...
        (
            ...
            "We embed the raw HyDE-expanded query with no preprocessing. We never tested whether normalising the query (lowercase, punctuation removal) improves retrieval on our KB — a basic test we skipped.",
        ...
        (
            ...
            "We cache nothing. The 9 suggested queries — clicked by most users — get re-embedded on every request. Pre-computing those 9 embeddings at startup would be a simple win we never took.",
        ...
        (
            ...
            "English only, undocumented. If a non-English-speaking PM types a query, the embedder processes it but retrieval quality against our English KB is unknown — we never tested cross-lingual queries.",
