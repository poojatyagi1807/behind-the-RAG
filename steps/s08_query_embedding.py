...
""", unsafe_allow_html=True)
    ...
    rows_data = [
        (
            ...
            "We use all-MiniLM-L6-v2 for both query and document embedding — same helper function, same model. But this is a code convention, not a version-locked config. If someone upgrades fastembed and the model changes silently, retrieval breaks with no alert.",
        ...
        (
            ...
            "all-MiniLM-L6-v2 takes ~200ms on first call (model load), then ~20ms per query. We never measured this against a latency budget. A PM should have defined: query embedding must complete in under 100ms to avoid user-perceived delay in the interactive demo.",
        ...
        (
            ...
            "We embed the raw HyDE-expanded query with no preprocessing. We never tested whether normalising the query (lowercase, punctuation removal) improves retrieval quality on our educational KB. A simple A/B test would have answered this.",
        ...
        (
            ...
            "No caching. Every query — including the 9 recommended ones many users click — gets re-embedded on every request. A PM should have identified: the 9 suggested queries are deterministic and should be pre-embedded and cached at startup.",
        ...
        (
            ...
            "English only, undocumented. If a PM types a query in Hindi or Spanish, the embedder handles it but retrieval quality against our English KB is unknown. We never tested cross-lingual queries. A PM should document this as a known limitation.",
