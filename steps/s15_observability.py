...
""", height=680)
    ...
    rows_data = [
        (
            ...
            "We capture no real metrics — the trace shown is a demo. We have no baseline record of retrieval scores, latency, or faithfulness from day one, so there's nothing to compare against when quality drifts.",
        ...
        (
            ...
            "Nothing is logged in production — no queries, no retrieval scores, no session data. We have no way to know what users are actually asking or how well the system is serving them.",
        ...
        (
            ...
            "No alerts exist. If the app starts returning low-confidence answers to every query, we find out only when a user reports it — there's no automated signal at all.",
        ...
        (
            ...
            "We have no layer-level instrumentation. A bad answer could come from retrieval failure, a stale KB, or a bad LLM response — we can't tell which without manually investigating every case.",
        ...
        (
            ...
            "We have no feedback button. Users who get a wrong answer have no way to signal it — and we have no mechanism to connect a user complaint to a specific chunk or pipeline step.",
        ...
        (
            ...
            "RLHF is taught in this step as a concept but not implemented in the app. We collect zero feedback signals — so the retrieval weights on day 1,000 are identical to day 1.",
        ...
        (
            ...
            "We have no monitoring. Gradual quality decay — from memory changes, model drift, or document staleness — would be invisible until a user notices and complains.",
