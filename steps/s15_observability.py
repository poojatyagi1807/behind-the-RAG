...
""", height=680)
    ...
    rows_data = [
        (
            ...
            "We show a demo trace but capture no real metrics. We have no baseline — no record of what retrieval scores, latency, or faithfulness looked like on day one. A PM should define health baselines from the first real user session and log them for comparison.",
        ...
        (
            ...
            "Nothing is logged in production. No query logs, no retrieval scores, no session data. A PM should decide: even for a demo app, log anonymised query intent and retrieval confidence so you can see what users actually ask and how well the system serves them.",
        ...
        (
            ...
            "No alerting. If the app starts returning low-confidence answers to every query, no one finds out until a user reports it. A PM should define: if average retrieval confidence drops below 0.3 for 3 consecutive sessions, send a notification to the owner.",
        ...
        (
            ...
            "We have no way to tell whether a bad answer was caused by retrieval failure, a stale KB, or a bad LLM response. A PM should instrument each layer separately so failure triage takes minutes, not investigation days.",
        ...
        (
            ...
            "No feedback mechanism. Users can't give thumbs up or down. We have no signal about which answers were helpful. A PM should add even a simple binary feedback button from day one — the signal is more valuable than the engineering cost.",
        ...
        (
            ...
            "No feedback capture at all. RLHF is taught conceptually but not implemented. A PM should at minimum log which recommended queries get used most — that alone tells you which RAG concepts are most confusing to the target audience.",
        ...
        (
            ...
            "No monitoring. If Streamlit Cloud changes its memory allocation and index quality degrades, we have no way to detect it. A PM should define a weekly manual check: run 5 standard queries and confirm answer quality meets the baseline set on day one.",
