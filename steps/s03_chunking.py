...
""", unsafe_allow_html=True)
    ...
    render_pm_matrix("Chunking Strategy", [
        ("What is your user asking?",
         ...
         "We used 400-token fixed chunks optimised for retrieval precision. But our users ask conceptual questions — 'why does chunking matter?' — not fact lookups, so fixed chunks often cut an explanation mid-thought.",
        ...
        ("How structured is your content?",
         ...
         "We applied the same chunking strategy to the academic RAG paper, the Lilian Weng blog, and LangChain API docs — completely different structures treated identically. Code snippets, flowing prose, and academic sections all got the same 400-token cut.",
        ...
        ("Does chunk boundary affect trust?",
         ...
         "We used word-based splitting with 75-token overlap, with no rule about where boundaries land. Some explanations — like how HNSW navigates layers — get cut mid-concept with no recovery.",
        ...
        ("What metadata travels with the chunk?",
         ...
         "We defined 15 metadata fields but never populated created_at or updated_at with real dates. A user asking 'what's the latest on RAG evaluation?' gets no freshness signal because the timestamps are empty strings.",
        ...
        ("Who validates chunk quality?",
         ...
         "The builder validated chunks alone — no one else reviewed them. We never tested whether a non-technical PM receiving a retrieved chunk could make sense of it without the surrounding context.",
