...
""", unsafe_allow_html=True)
    ...
    render_pm_matrix("Chunking Strategy", [
        ("What is your user asking?",
         ...
         "We used 400-token fixed chunks optimised for retrieval. But our users ask conceptual questions like 'why does chunking matter?' not fact lookups. Fixed chunking doesn't preserve a full explanation in one chunk. We should have tested semantic chunking against our actual query types first.",
        ...
        ("How structured is your content?",
         ...
         "We applied the same chunking strategy to the academic RAG paper, the Lilian Weng blog, and LangChain API docs — very different structures. The paper has abstract/sections, the blog has flowing prose, the API docs have code snippets. One strategy for all three was the wrong call.",
        ...
        ("Does chunk boundary affect trust?",
         ...
         "We used word-based splitting with 75-token overlap. Some explanations — like the HNSW algorithm description — get cut mid-concept. A PM should have defined: no chunk may end mid-sentence on a concept that requires a complete definition to be useful.",
        ...
        ("What metadata travels with the chunk?",
         ...
         "We defined 15 metadata fields but created_at and updated_at were never populated with real dates. A user asking 'what's the latest on RAG evaluation?' can't be served freshness filtering because the timestamps are empty.",
        ...
        ("Who validates chunk quality?",
         ...
         "The builder validated chunks alone. No domain expert reviewed whether a non-technical PM could make sense of the retrieved chunks. We should have run a 5-person test: give 5 chunks to a non-technical PM and ask if the answers make sense in context.",
