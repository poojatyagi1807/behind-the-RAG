...
""", unsafe_allow_html=True)
    ...
    rows_data = [
        (
            ...
            "We show source document title as a citation. But many chunks come from blog posts and papers that a non-technical PM can't easily evaluate. We should have added a 'source type' field (research paper / framework doc / blog) so users can judge source credibility at a glance.",
        ...
        (
            ...
            "We have a document_type field but it's not used to filter queries automatically. A user asking 'how do I evaluate RAG?' should trigger a filter to RAGAS docs only. That filter logic exists conceptually but we never defined which query intents map to which filters.",
        ...
        (
            ...
            "Auto-generated at chunk time using content detection heuristics. But has_tables and has_citations were set by rules, not verified. A PM should have spot-checked 20 chunks to confirm metadata was being set correctly before trusting it for filtering.",
        ...
        (
            ...
            "created_at and updated_at fields exist in the schema but are never populated. If we add a newer version of the RAGAS docs, there's no way to distinguish it from the old version. A PM should have required real timestamps before calling the metadata schema complete.",
        ...
        (
            ...
            "Users see the source document title in search results. They don't see the document type, date, or section. A PM should have asked: what's the minimum provenance info a learner needs to trust a retrieved answer? We never defined that.",
