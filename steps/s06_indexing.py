...
""")
    ...
    rows_data = [
        (
            ...
            "We used usearch in-memory — no persistence. Every session starts fresh and re-indexes from scratch. A PM should have asked: does this app need a persistent index? The answer is yes — we're making every user wait for indexing on every visit.",
        ...
        (
            ...
            "In-memory usearch runs on Streamlit Cloud with no configuration overhead. But it means zero persistence and unknown memory limits. We never checked: what's the memory ceiling on Streamlit Cloud free tier? What happens when the KB grows beyond it?",
        ...
        (
            ...
            "No latency SLA defined. The HNSW index builds on first load, taking several seconds. A PM should have defined: maximum acceptable wait on first load is X seconds. We shipped with no measurement or user communication about the delay.",
        ...
        (
            ...
            "We have 312 chunks from 5 documents. We never asked: what's the plan if we expand to 20 documents? Does usearch in-memory scale? At what point do we need Pinecone or Weaviate? No scaling plan means a painful migration when the time comes.",
        ...
        (
            ...
            "We built hybrid search (dense + BM25 via RRF) from the start. But we never measured whether it outperforms dense-only on our specific queries. A PM should have run a retrieval quality comparison before committing to hybrid as the default.",
