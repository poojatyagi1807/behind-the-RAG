...
""")
    ...
    rows_data = [
        (
            ...
            "We used usearch in-memory with no persistence. Every session rebuilds the entire index from scratch, making every user wait for something that could have been built once and reused.",
        ...
        (
            ...
            "We chose in-memory usearch for zero setup overhead — but never checked the memory ceiling on Streamlit Cloud. When the KB grows, there's no migration plan and no warning before the index breaks.",
        ...
        (
            ...
            "We set no latency target. The HNSW index builds on first load and takes several seconds — we shipped with no measurement and no communication to the user about why they're waiting.",
        ...
        (
            ...
            "We have 312 chunks from 5 documents and no growth plan. We don't know whether usearch in-memory scales if we add more documents, or at what point we'd need to migrate to a persistent vector database.",
        ...
        (
            ...
            "We built hybrid search (dense + BM25 via RRF) from the start without measuring whether it outperforms dense-only on our actual queries. We committed to hybrid before confirming it helps.",
